...
if __name__ == '__main__':
  ...
  cursor.execute('CREATE INDEX wd_wikidata_wikidata_id ON wd.wikidata(wikidata_id)')
  cursor.execute('CREATE INDEX wd_wikidata_properties ON wd.wikidata USING gin(properties)')
  cursor.execute('CREATE INDEX wd_wikidata_labels ON wd.wikidata USING gin(labels)')
  cursor.execute('CREATE INDEX wd_wikidata_sitelinks ON wd.wikidata USING gin(sitelinks)')
  conn.commit()
  cursor.execute('DROP TABLE IF EXISTS wd.geo')
  cursor.execute('CREATE TABLE wd.geo ('
                 '    wikidata_id TEXT,'
                 '    geometry geometry(POINT, 4326)'
                 ')')
  cursor.execute('INSERT into wd.geo (wikidata_id, geometry) '
                ...
                'FROM wd.wikidata WHERE properties->\'coordinate location\' IS NOT NULL;'
                )
  cursor.execute('CREATE INDEX wd_geo_geometry ON wd.geo USING gist (geometry) TABLESPACE pg_default;')
  conn.commit()
  cursor.execute('DROP TABLE IF EXISTS wd.labels')
  cursor.execute('CREATE TABLE wd.labels ('
                 '    wikidata_id TEXT,'
                 '    label TEXT'
                 ')'
                 )
  cursor.execute('INSERT INTO wd.labels (wikidata_id, label) SELECT wikidata_id, jsonb_array_elements_text(labels) '
                 'FROM wd.wikidata;'
                )
  cursor.execute('CREATE INDEX wd_wikidata_labels_trgm ON wd.labels USING gist (label COLLATE pg_catalog."default" gist_trgm_ops) TABLESPACE pg_default;')
  conn.commit()

  cursor.execute('DROP TABLE IF EXISTS wd.instance')
  cursor.execute('CREATE TABLE wd.instance ('
                 '    wikidata_id TEXT,'
                 '    instance_of TEXT'
                 ');'
                 )
  cursor.execute('INSERT INTO wd.instance (wikidata_id, instance_of) '
                 'SELECT wikidata_id, lower(properties->>\'instance of\')::jsonb '
                 'FROM wd.wikidata WHERE jsonb_typeof(properties->\'instance of\') = \'array\';'
                 )
  cursor.execute('INSERT INTO wd.instance (wikidata_id, instance_of) '
